...
class InputSystem:
    ...
    def update(self, dt):
        if self.game_state.is_minigame_active:
            return
        ...
        # [NEW] Action Keys (Q: Skill/Siren, R: Reload/Passive?, V: Attack)
        # InputSystem update loop checks just pressed? No, better handle in event loop or check debounce
    ...
    def handle_event(self, event):
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_q:
                self.event_bus.publish("ACTION_REQUEST", {'type': 'SKILL', 'entity_id': pid})
            elif event.key == pygame.K_v:
                self.event_bus.publish("ACTION_REQUEST", {'type': 'ATTACK', 'entity_id': pid})
            elif event.key == pygame.K_f:
                # Flashlight toggle
                inv = self.ecs.get_component(pid, Inventory)
                if inv: inv.flashlight_on = not inv.flashlight_on 
