...
class VisualSoundEffect:
    def __init__(self, x, y, text, color, size_scale=1.0, duration=1500, shake=False, blink=False):
        ...
        # OLD Logic Restoration
        angle_deg = random.uniform(240, 300)
        self.angle_rad = math.radians(angle_deg)
        self.speed = 1.2 * size_scale
        
        self.resource_manager = ResourceManager.get_instance()
        # OLD Font Logic: base_size calculation
        base_size = int(max(16, (52 * size_scale) * 0.5))
        
        # Use 'arial black' to match OLD exactly. If system font fails, fallback handled by pygame usually.
        # OLD used a caching mechanism, here we rely on ResourceManager or create new.
        # To be safe and exact:
        try:
            self.font = pygame.font.SysFont("arial black", base_size, bold=True)
        except:
            self.font = pygame.font.SysFont("arial", base_size, bold=True)
